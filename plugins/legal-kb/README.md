...
운영자에게 로그인한 이메일과, 웹 `플러그인 연동` 화면에 보이는 UID 를 함께 전해 등록을 요청한다.
...
- 수록 범위 — **국가법령 전체**(법률·대통령령·총리령·부령 등). 민법·상법·형법·근로기준법과 세법을 포함한다. 행정규칙·자치법규는 없다

스킬은 켜지기 전까지 이름과 한 줄 설명(30~50 토큰)만 쓴다. 법률 질문(세법 포함)이 들어올 때만 본문이 실린다.
